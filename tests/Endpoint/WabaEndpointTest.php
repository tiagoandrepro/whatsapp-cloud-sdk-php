...
final class WabaEndpointTest extends TestCase
{
    public function testGetOwnedWabas(): void
    {
        $response = new Response(200, [], json_encode(['data' => []], JSON_THROW_ON_ERROR));
        $client = new MockPsr18Client([$response]);
        $endpoint = new WabaEndpoint($this->createTransport($client), '123');

        $endpoint->getOwnedWabas('business');

        $request = $client->getLastRequest();
        self::assertNotNull($request);
        self::assertSame(
            'https://graph.facebook.com/v24.0/business/owned_whatsapp_business_accounts',
